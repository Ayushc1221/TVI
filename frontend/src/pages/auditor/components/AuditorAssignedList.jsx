...
import { CheckCircle, FileText, UploadCloud, AlertCircle, Loader2 } from 'lucide-react';
    ...
    if (selectedAudit) {
        return (
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                <button onClick={() => setSelectedAudit(null)} className="text-slate-500 hover:text-slate-800 mb-4 font-semibold text-sm flex items-center gap-1">
                    &larr; Back to List
                </button>
                <div className="flex items-center gap-3 mb-6">
                    <FileText className="w-6 h-6 text-blue-600" />
                    <div>
                        <h2 className="text-xl font-bold text-slate-800">Submit Audit Report</h2>
                        <p className="text-sm text-slate-500">For {selectedAudit.companyName} ({selectedAudit.applicationId})</p>
                    </div>
                </div>

                <form onSubmit={handleSubmitReport} className="max-w-xl space-y-6">
